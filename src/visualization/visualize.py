...
def distribution_of_energy_star_score(features2):
    ...
    plt.figure(figsize=(10, 6))
    fig_2 = sns.histplot(merged_data[features2], bins=20, kde=True, color='skyblue')
    plt.title('Distribution of ENERGY STAR Scores')
    plt.xlabel('ENERGY STAR Score')
    plt.ylabel('Count')
    plt.savefig(filepath)
    plt.close()
distribution_of_energy_star_score(df['Energy_Star_1-100_Score'])
...
def box_plot_energy_efficiency(features1,features2):
    ...
    plt.savefig(filepath)
    plt.close()
box_plot_energy_efficiency(df['Energy_Efficiency_Grade'],df['DOF_Gross_Square_Footage'])

# Box plot of Energy Efficiency Grade vs. ENERGY STAR Score
plt.figure(figsize=(10, 6))
fig_4 = sns.boxplot(x='Energy_Efficiency_Grade', y='Energy_Star_1-100_Score', data=merged_data, palette='Set2')
